...
const HighlightPrize: React.FC<HighlightPrizeProps> = ({
    ...
    const cardClasses = clsx(
        'rounded-2xl border bg-white/5 p-6 text-center shadow-xl backdrop-blur-md',
        'transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl',
        {
            'border-blue-500/50 bg-blue-500/10 shadow-blue-500/20': isMainPrize,
            'border-white/10': !isMainPrize,
        }
    );

    const numberClasses = clsx(
        'my-4 text-5xl font-bold tracking-wider sm:text-6xl',
        {
            'text-blue-400': isMainPrize,
            'text-white': !isMainPrize,
        }
    );
    ...
    return (
        <div className={cardClasses}>
            ...
            <div className={numberClasses}>{prize.number.join('   ')}</div>
